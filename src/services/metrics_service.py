...
def compute_package_rating(url: str) -> PackageRating:
    ...
    return PackageRating(
        ...
        size_score=SizeScore(
            raspberry_pi=get_res("size")[0].get("raspberry_pi", 0.0) if isinstance(get_res("size")[0], dict) else get_res("size")[0],
            jetson_nano=get_res("size")[0].get("jetson_nano", 0.0) if isinstance(get_res("size")[0], dict) else get_res("size")[0],
            desktop_pc=get_res("size")[0].get("desktop_pc", 0.0) if isinstance(get_res("size")[0], dict) else get_res("size")[0],
            aws_server=get_res("size")[0].get("aws_server", 0.0) if isinstance(get_res("size")[0], dict) else get_res("size")[0]
        ),
